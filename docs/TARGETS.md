...
| `target_configuration_failure` | Expected service not TESTABLE / not detected |
...
plus the same `checker_id`, `binding_id`, and `catalog_id` from the B3 support contract. Helix does not recompile a per-target checker.
