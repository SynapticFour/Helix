...
**Maturity: Early stage.** Built alongside Ferrum. `helix verify` currently runs **DRS** checks (the five HelixTest DRS names in [INVENTORY.md](INVENTORY.md)). WES, TES, TRS, and htsget are discovered when they answer; those checks are **not** executed yet. `helix security` and `helix bench` are started scaffolds, not stage exits. Results are not GA4GH certification.
